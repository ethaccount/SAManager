			...
			<div class="space-y-3">
				...
				<div v-else class="flex flex-col p-3 border rounded-lg border-border/50 bg-background/50">
					<div class="flex items-center gap-2">
						...
						<span class="text-sm text-muted-foreground/70"> EOA Wallet not supported by this browser </span>
					</div>
				</div>
				...
				>
					<div v-if="!isLogin" class="flex justify-between items-center">
						...
						<Button variant="outline" size="sm" class="h-8 text-sm px-3" @click="openConnectPasskeyBoth">
							Connect
						</Button>
					</div>
					<div v-if="isLogin">
						<div class="flex justify-between items-center">
							<div class="flex items-center gap-2">
								<CheckCircle
									class="w-4 h-4"
									:class="
										selectedSigner?.type === 'Passkey' ? 'text-green-500' : 'text-muted-foreground'
									"
								/>
				...
				>
					<div>
						<div class="flex justify-between items-center mb-2">
							<Address :address="account.address" text-size="sm" button-size="xs" :show-buttons="false" />
