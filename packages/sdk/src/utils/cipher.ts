import type { EncryptedData } from '@/message'
import type { RPCRequest, RPCResponse } from '@/message'
import { hexStringToUint8Array, uint8ArrayToHex } from '@/utils'
...
export async function exportKeyToHexString(type: 'public' | 'private', key: CryptoKey): Promise<string> {
	const format = getFormat(type)
	const exported = await crypto.subtle.exportKey(format, key)
	return uint8ArrayToHex(new Uint8Array(exported))
}

export async function importKeyFromHexString(type: 'public' | 'private', hexString: string): Promise<CryptoKey> {
	const format = getFormat(type)
